"""SignalWire number provisioning via Compatibility API."""
...
from app.providers.capability_presets import runtime_caps, signalwire_numbers
...
class SignalWireNumberProvisioningProvider(NumberProvisioningProvider):
    @property
    def provider_name(self) -> str:
        return "signalwire"

    def is_configured(self) -> bool:
        return signalwire_client.is_phone_provisioning_configured()

    def get_capabilities(self) -> ProviderCapabilities:
        return runtime_caps(signalwire_numbers(), self, service="numbers")
    ...
    def search_numbers(
        ...
        *,
        ...
        return signalwire_client.search_available_phone_numbers(
            country_code,
            prefix=prefix,
            limit=limit,
    ...
    def purchase_number(self, phone_number: str) -> ProviderResult:
        if not self.is_configured():
            raise ProviderUnavailableError(provider=self.provider_name)
        order = signalwire_client.create_number_order(phone_number)
    ...
    def wait_for_purchase(self, order_id: str, *, timeout_seconds: int = 45) -> ProviderResult:
        try:
            order = signalwire_client.wait_for_number_order(
                order_id,
    ...
    def find_number_record(self, phone_number: str) -> dict[str, Any] | None:
        return signalwire_client.find_phone_number_record(phone_number)

    def release_number(self, provider_number_id: str) -> ProviderResult:
        if not self.is_configured():
            raise ProviderUnavailableError(provider=self.provider_name)
        signalwire_client.release_phone_number(provider_number_id)
    ...
    def configure_voice(self, provider_number_id: str) -> ProviderResult:
        ...
        result = signalwire_client.configure_phone_number(provider_number_id, voice_url=voice_url)
        return ProviderResult(
            provider=self.provider_name,
    ...
    def configure_sms(self, provider_number_id: str) -> ProviderResult:
        ...
        result = signalwire_client.configure_phone_number(provider_number_id, sms_url=sms_url)
        return ProviderResult(
            provider=self.provider_name,
            external_id=provider_number_id,
            data=result,
